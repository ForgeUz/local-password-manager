...
class _SettingsScreenState extends State<SettingsScreen> {
  ...
  Future<void> _exportVault() async {
    ...
    try {
      ...
      setState(() => _exportStatus =
          e.code == 'cancelled' ? 'Export cancelled.' : 'Export failed: ${e.message}');
    } catch (e, stack) {
  ...
  Future<void> _importVault() async {
    ...
    try {
      ...
      debugPrint('Import: first 16 bytes: ${bytes.sublist(0, bytes.length > 16 ? 16 : bytes.length)}');
      ...
      debugPrint('Import: password entered, length=${mpController.text.length}');
      ...
      debugPrint('Import: wrote to $tmpPath, size=${await File(tmpPath).length()}');
      ...
      if (mounted) {
        setState(() => _importStatus = '✓ Vault imported. Unlock with its master password.');
        Navigator.pop(context);
      }
    } on PlatformException catch (e) {
      debugPrint('Import PlatformException: ${e.code} - ${e.message}');
      setState(() => _importStatus =
        e.code == 'cancelled' ? 'Import cancelled.' : 'Import failed: ${e.message}');
    } catch (e, stack) {
  ...
  Future<void> _generateShares(BuildContext context) async {
    ...
    try {
      final shares = await widget.service.generateShares(mp, n: _shamirN, k: _shamirK);
    ...
    return Scaffold(
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            const Text('Backup', style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold)),
