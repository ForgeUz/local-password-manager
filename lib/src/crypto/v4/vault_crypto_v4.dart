...
class VaultCryptoV4 {
  ...
  // primary vault (v5 E3: one salt). duressUnlockSession derives MK_duress from
  // the primary header's salt, so the decoy must be locked with that salt.
  ...
  // Re-lock with a held VRK (no Argon2id re-derivation). Used by VaultService
  // after an edit. Generates fresh salt/nonce.
  // Re-lock with a held VRK (no Argon2id re-derivation). Preserves the salt +
  // KDF params from the incoming blob so a future MP unlock/reauth re-derives
  // the same VRK (deterministic). Fresh nonce per re-encryption.
  // Re-lock with a held VRK, preserving the salt + KDF params from the incoming
  // blob so a future MP unlock/reauth re-derives the same VRK (deterministic).
  ...
  Uint8List _buildBlob(
    ...
    // Slot 2 (v5 E3/E4): decoy vault blob if deniability enabled, else CSPRNG
    // noise. Identical structure either way — one file, two slots.
  ...
  // Unlock and return a held-VRK session (SecureBuffer + entries + search tags).
  // Wrong MP fails at the outer-GCM tag (DecryptionFailedError), fail-closed.
  ...
  // v5 E2: unlock via a released SecondFactorMaterial (SFM). The SFM is
  // decrypted from the outside-vault file under MK_base, then folded into the
  // REAL KDF path (same as a TOTP code). No bypass branch exists.
  ...
  // v5 E2: unlock via a valid backup code. Derives MK_base, opens the SFM file
  // (code-gated, single-use, rate-limited), releases SFM into the KDF, then
  // decrypts through the REAL path. Wrong/consumed/rate-limited code throws
  // BackupCodeError BEFORE any vault decrypt. Returns the session + the updated
  // SFM file (code consumed / attempt recorded) for the caller to persist.
  ...
  // VRK, recomputes ALL search_tags, rewrites header + all payloads, and
  // re-seals the SFM file under the new MK_base (seed untouched — 2FA survives).
  // The caller persists the returned blob + sfmFile via temp-file-then-rename.
  // Wrong old MP -> DecryptionFailedError (verified before any rewrite).
  // Invariants: no mixed tag/VRK state; old MP fails after change; new MP +
  // TOTP and new MP + backup code both succeed.
  Future<MpChangeResult> changeMasterPassword(Uint8List blob,
    ...
    // Old MK_base + (if 2FA) reveal SFM under it. Verify old MP by decrypting
    // the vault through the REAL path — wrong old MP fails here, before any
    // rewrite.
    ...
    // Preserve slot 2 (decoy vault or noise). Locked under VRK_duress derived
    // from the SAME salt -> still valid after the change.
    final slot2 = slot2Of(blob);

    // Atomic rewrite: fresh DEKs wrapped under new VRK, fresh search_tags under
    // new SearchKey, fresh nonce. One blob, no mixed state.
  ...
  // Unlock with a duress-derived VRK. Derives MK_duress from the same MP and
  // salt via Argon2id, then VRK_duress = HKDF(MK_duress). If the decoy vault
  // (second slot) decrypts under VRK_duress, returns the session; otherwise
  // throws DuressDecryptError (the second slot is random noise when deniability
  // is disabled, so this fails for a wrong duress MP too).
  ...
  // Encode a string entry id into a fixed 16-byte UUID field. Pads with zeros
  // if shorter; truncates if longer. The id is opaque to the crypto layer.
