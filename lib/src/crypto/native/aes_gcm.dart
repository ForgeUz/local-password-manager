...
typedef EncryptNative = Int32 Function(
    ...
    Pointer<Void>,
    Pointer<Void>);
typedef EncryptDart = int Function(Pointer<Void>, Pointer<Void>, Pointer<Void>,
    int, Pointer<Void>, int, Pointer<Void>, Pointer<Void>, Pointer<Void>);
typedef DecryptNative = Int32 Function(
    ...
    Pointer<Void>,
    Pointer<Void>);
typedef DecryptDart = int Function(Pointer<Void>, Pointer<Void>, Pointer<Void>,
    Pointer<Void>, int, Pointer<Void>, int, Pointer<Void>, Pointer<Void>);
...
class AesGcm {
  ...
  static Uint8List encrypt(
      Uint8List key, Uint8List nonce, Uint8List aad, Uint8List pt) {
    _ensureInit();
    final lib = DynamicLibrary.open(
        Platform.isAndroid ? 'libsodium.so' : 'libsodium.so.23');
    final enc = lib.lookupFunction<EncryptNative, EncryptDart>(
  ...
  static Uint8List decrypt(
      Uint8List key, Uint8List nonce, Uint8List aad, Uint8List ct) {
    ...
    final lib = DynamicLibrary.open(
        Platform.isAndroid ? 'libsodium.so' : 'libsodium.so.23');
    final dec = lib.lookupFunction<DecryptNative, DecryptDart>(
  ...
  static void _ensureInit() {
    ...
    final lib = DynamicLibrary.open(
        Platform.isAndroid ? 'libsodium.so' : 'libsodium.so.23');
    final isAvailable =
        lib.lookupFunction<AesAvailableNative, AesAvailableDart>(
            'crypto_aead_aes256gcm_is_available')();

    if (isAvailable == 0) {
      throw StateError(
          'AES-GCM hardware acceleration (AES-NI) not available on this CPU. '
